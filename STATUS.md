...
## อัปเดตรอบ 7 (25 ก.ค. 2026) — วิดีโอชุดใหม่ + ย้ายรูปตามที่ลูกค้าสั่ง

| จุด | ไฟล์ | หมายเหตุ |
|---|---|---|
| hero **หน้าแรก** (TH+EN) | `maid-service-overview-phuket.mp4` ← `main.mp4` | มอนทาจรวมหลายบริการ ตัดแถบดำออกแล้ว (406×556) แทนรูปนิ่งเดิม |
| hero **เช็ดกระจกอาคาร** | `window-glass-washing-phuket.mp4` ← `Glass-clean.mp4` | ทับไฟล์เดิม (ชื่อยังตรงเนื้อหา) ไม่ต้องแก้ reference · 406×722 |
| hero **แม่บ้านรายชั่วโมง** | `hourly-maid-cleaning-living-room-phuket.webp` ← `S__7422052_0.jpg` | แทนรูปเดิมที่เป็นแม่บ้านนั่งพื้นกับถัง |

- วิดีโอทุกไฟล์ตัด audio ออก (autoplay muted loop) + `preload="metadata"` + poster WebP → poster เป็น LCP ไม่กระทบความเร็ว
- ⚠️ **ค้างไว้:** รูป `S__7422052` ตอนนี้ถูกใช้เป็น hero ทั้งหน้า **แม่บ้านรายชั่วโมง** และ **Deep Cleaning** (คนละไฟล์ แต่ภาพเดียวกัน) — 2 ใน 4 หน้า Ads หน้าตาเหมือนกัน ถ้าจะแก้ต้องเลือกรูป hero ใหม่ให้ Deep Cleaning
- ⚠️ วิดีโอ `main.mp4` มีลายน้ำ "Happy Weekend" ติดมาในไฟล์ต้นฉบับ และเสื้อทีมในคลิปเป็นแบรนด์ "Steaming Cleaner Phuket" (เบอร์เดียวกัน) ไม่ใช่เสื้อส้ม PHUKET MAIDS
- ตรวจ: build 83 หน้า · media ครบ · ลิงก์ 140 URL · i18n ครบคู่ · ไม่มี console error · เช็คทั้ง desktop/mobile

## อัปเดตรอบ 6 (25 ก.ค. 2026) — เปลี่ยนรูป/วิดีโอเป็นชุดใหม่จากลูกค้า

ลูกค้าส่งรูปใหม่ 9 + วิดีโอ 3 มาที่ `asset/` เลือกเฉพาะที่**ดีกว่าของเดิมจริง** มาแทน (เทียบแบบ crop 4:3 ตามที่เว็บแสดงจริงก่อนตัดสินใจ)

| ไฟล์ในเว็บ | รูปใหม่ | เหตุผล |
|---|---|---|
| `living-room-deep-cleaning-phuket.webp` | S__7422052 | ห้องนั่งเล่นสว่าง พื้นเงาสะท้อน + พนักงานเสื้อส้ม — hero ของ Deep Cleaning (ใช้ 16 จุด) |
| `window-squeegee-cleaning-phuket.webp` | S__7422061 | กระจกบานสูงพร้อมไม้ต่อด้าม (ของเดิมย้อนแสง มีของรกหน้าภาพ) |
| `kitchen-top-cabinet-cleaning-phuket.webp` | S__7422060 | เช็ดตู้ครัวบนสุด เห็นยูนิฟอร์มชัด (ของเดิมมืดและรก) |
| `kitchen-deep-cleaning-phuket.webp` | S__7422055 | ทีม 2 คนเช็ดตู้เย็น สว่างกว่าเดิม |
| `bathroom-shower-…` → `bathroom-deep-cleaning-phuket.webp` | S__7422053 | **ของเดิมเบลอ/ฝ้าจนดูไม่ออก** — เปลี่ยนชื่อไฟล์ให้ตรงภาพใหม่ |
| `pool-villa-cleaning-phuket.mp4` (ใหม่) | วิดีโอ 806664266.446492 | หน้าพูลวิลล่าเคยใช้รูปห้องน้ำเบลอเป็น hero → เปลี่ยนเป็นวิดีโอทีมซักโซฟาในพูลวิลล่าหรู (ตัดแถบดำออก, 406×548) |

- แก้ alt/caption ทุกจุดที่ภาพใหม่ทำให้ข้อความเดิมไม่ตรง (เช่น "ขึ้นบันได", "บ้านโล่ง", "ขัดคราบฉากอาบน้ำ") — รวม 18 จุด TH+EN
- **ไม่ได้ใช้:** วิดีโอ 3s (สั้นไป), วิดีโอ 11s (ภาพสั่น/เบลอ), รูป 51/54/58/59 (ไม่ได้ดีกว่าของเดิมในสล็อตที่มีอยู่ — 59 เป็นภาพล้างแอร์ ซึ่งเว็บยังไม่ได้ขายบริการนี้)
- ตรวจ: build 83 หน้าผ่าน · ไฟล์สื่อที่อ้างอิงครบทุกไฟล์ · ลิงก์ 137 URL ไม่เสีย · i18n ครบคู่

## อัปเดตรอบ 5 (23 ก.ค. 2026) — แปลภาษาอังกฤษครบทั้งเว็บ

แปล service ย่อย 21 หน้า + location 11 หน้า เป็น EN เสร็จครบ → **เว็บสองภาษาสมบูรณ์ทั้งไซต์**

- **หน้าทั้งหมดมีคู่ EN แล้ว**: service 25 หน้า + location 11 หน้า + หน้าหลัก 6 หน้า ครบทั้งไทยและอังกฤษ
- เพิ่ม route `/en/locations/[...slug]/` + areaLinks section ในหน้า service EN + footer สลับพื้นที่เป็น /en/locations เมื่อภาษาอังกฤษ
- ปุ่มสลับภาษาทำงานครบทุกทิศ: service↔, location↔, หน้าหลัก↔ (ทุกหน้ามีหน้าคู่จริง ไม่มี fallback แล้ว)
- แปลด้วย subagents 5 ตัวขนานกัน (อ้างอิงไฟล์ EN pillar เป็นแม่แบบ) — เนื้อหา unique ทุกหน้า ราคาตรงต้นฉบับ ลิงก์ในเนื้อหาชี้ /en/ หมด
- **ยอดหน้า: 83 หน้า** (42 ไทย + 41 อังกฤษ; thank-you EN ไม่เข้า sitemap)
- ตรวจครบ: build ผ่าน, check-i18n (ทุกหน้ามีคู่ + ลิงก์ EN ชี้ /en หมด), link check 136 URL ไม่เสีย, sitemap 80 URL (40 TH + 40 EN), uniqueness 72 หน้า content ไม่ซ้ำเกิน 10%
- hreflang alternate (th↔en) ครบทุกหน้าที่มีคู่
...
1. **รีวิวจริงจากลูกค้า** — เอา placeholder `[รอรีวิวจริงจากลูกค้า]` ออกแล้ว เปลี่ยนเป็นบล็อก "สิ่งที่ลูกค้าบอกเราบ่อยที่สุด" (สรุปฟีดแบ็ก ไม่อ้างชื่อ/ไม่โชว์ดาว) เพื่อไม่ให้ผิดนโยบาย Misrepresentation ของ Google Ads · **เมื่อได้รีวิวจริง** (ผ่าน Google Business Profile หรือขอจากลูกค้าเก่า) ค่อยเพิ่มการ์ดรีวิว + `AggregateRating` schema ได้ — ห้ามใส่ข้อความที่แต่งขึ้นเองไม่ว่าจะระบุชื่อหรือไม่
2. ~~**โลโก้จริง**~~ ✅ เสร็จแล้ว — วาดโลโก้แม่บ้านถือไม้ปัดฝุ่นบนวงกลมขาวเป็น SVG ตามโลโก้ที่ลูกค้าส่งมา (`site/src/components/Logo.astro` + `public/favicon.svg`) พร้อมสร้าง favicon.ico / apple-touch-icon ใหม่ด้วย `node scripts/gen-icons.mjs` — ถ้ามีไฟล์ต้นฉบับ AI/EPS ความละเอียดสูงส่งมาเพิ่มได้ จะได้เก็บรายละเอียดตรงต้นฉบับ 100%
